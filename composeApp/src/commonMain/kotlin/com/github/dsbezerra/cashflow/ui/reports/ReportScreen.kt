...
fun ReportScreen(viewModel: ReportViewModel = koinViewModel()) {
    ...
    Box(Modifier.fillMaxSize()) {
        if (state.isLoading) {
            ...
            ReportContent(state = state, onAction = viewModel::onAction)
        }
        SnackbarHost(
    ...
    Box(Modifier.fillMaxSize()) {
        LazyColumn(
            ...
            if (state.selectedTab == ReportTab.BY_PERIOD) {
                item {
                    PeriodSummaryCards(data)
                }
                if (data.totalIncome == 0.0 && data.totalExpenses == 0.0) {
                    item {
                        EmptyState("Nenhuma transação no período selecionado")
                    }
                } else {
                    item {
                        PeriodExtraStats(data)
                    }
                    if (data.dailyCumulative.isNotEmpty()) {
                        item {
                            DailyCumulativeLineChart(data.dailyCumulative)
                        }
                    }
                ...
                if (state.isDreLoading) {
                    ...
                    if (dre == null) {
                        item { EmptyState("Erro ao carregar DRE") }
...
private fun PeriodSummaryCards(data: ReportData) {
    Row(
        ...
        SummaryCard(
            ...
            color = if (data.netBalance >= 0) incomeColor else expenseColor,
            modifier = Modifier.weight(1f),
        )
    }
}
...
private fun CategoryBreakdownSection(
    ...
    Column(modifier = Modifier.fillMaxWidth()) {
        ...
        CategoryBreakdownList(items = items, color = color)
    }
}
...
private fun CategoryBreakdownList(items: List<CategoryAmount>, color: Color) {
    ...
    Column(verticalArrangement = Arrangement.spacedBy(12.dp)) {
        items.forEach { item ->
            CategoryAmountRow(item = item, maxAmount = maxAmount, color = color)
            HorizontalDivider()
        }
    }
}
...
private fun DreStatement(dre: DreReport) {
    Column(
        ...
        DreResultRow(label = "(-) Deduções", item = dre.deductions, color = expenseColor)
        DreTotalRow(label = "(=) Receita Líquida", amount = dre.netRevenue)
        HorizontalDivider(modifier = Modifier.padding(vertical = 4.dp))
        DreResultRow(label = "(-) Custos", item = dre.costs, color = expenseColor)
        DreTotalRow(label = "(=) Lucro Bruto", amount = dre.grossProfit)
        HorizontalDivider(modifier = Modifier.padding(vertical = 4.dp))
        DreSection(
            sign = "(-)",
...
private fun DreTotalRow(label: String, amount: Double, highlight: Boolean = false) {
    val color = if (amount >= 0) incomeColor else expenseColor
    Row(
        modifier = Modifier
            .fillMaxWidth()
            .padding(vertical = 8.dp),
