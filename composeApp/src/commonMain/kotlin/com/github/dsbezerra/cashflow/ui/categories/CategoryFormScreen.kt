    ...
    Scaffold(
        ...
        Box(
            ...
            CategoryDropdown(
                ...
                onSelect = { viewModel.onAction(CategoryFormAction.IconChanged(it)) },
            )
            ...
            if (state.availableParents.isNotEmpty()) {
                ...
                CategoryDropdown(
                    ...
                    selectedValue = parentOptions.firstOrNull { it.first == state.parentId }?.second ?: "Sem categoria pai",
                    options = parentOptions.map { it.second },
                    onSelect = { selected ->
                        val parentId = parentOptions.firstOrNull { it.second == selected }?.first
                        viewModel.onAction(CategoryFormAction.ParentChanged(parentId))
                    },
                )
            }
