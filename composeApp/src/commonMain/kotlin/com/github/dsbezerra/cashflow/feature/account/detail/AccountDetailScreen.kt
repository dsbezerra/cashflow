    ...
    LaunchedEffect(Unit) {
        viewModel.events.collectLatest { event ->
            when (event) {
                ...
                is AccountDetailEvent.NavigateToEdit -> onNavigateToEdit(
                    event.accountId
                )

                is AccountDetailEvent.ShowError -> snackbarHostState.showSnackbar(
                    event.message
                )
            }
        }
            ...
            TopAppBar(
                ...
                actions = {
                    if (state.account != null) {
                        IconButtonWithTooltip(onClick = { onNavigateToEdit(accountId) }, tooltip = "Editar") {
                            Icon(Icons.Default.Edit, contentDescription = "Editar")
                        }
                        if (!state.account!!.isArchived) {
                            IconButtonWithTooltip(onClick = { showArchiveConfirm = true }, tooltip = "Arquivar") {
